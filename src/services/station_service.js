...
exports.getAvgFillingRateByIdByDay = async function (idStation, day) {
	...
	const stats = await StatsByStationByHour.find({
		...
		{ date: { $gte: dateOfDay.clone().subtract(7, 'days'), $lt: dateOfDay.clone().subtract(6, 'days') } }, 
		{ date: { $gte: dateOfDay.clone().subtract(14,'days'), $lt: dateOfDay.clone().subtract(13,'days') } }
		],
	});
	for (let timeSlot = 0; timeSlot<24; timeSlot += 1 ){
		...
		0
			...
			dataAvgFillingRate.push({timeSlot : timeSlot, avgFillingRate: sumFillingRateByTimeSlot/3});
	}
